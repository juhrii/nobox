...
# Inject dump
target = r"final data = await chatProvider\.getDetailRoom\(widget\.chat\.id\);"
replacement = """final data = await chatProvider.getDetailRoom(widget.chat.id);
      try {
        File('C:\\\\Users\\\\LENOVO\\\\room_data_dump.txt').writeAsStringSync(jsonEncode(data));
      } catch(e) {
        print("Error dump: $e");
      }"""
